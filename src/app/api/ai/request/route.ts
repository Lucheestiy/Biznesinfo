...
function buildHairdresserSalonReply(message = ""): string {
  ...
  const lines = [
    ...
    ...(locationLabel ? [`Локация в контексте: ${locationLabel}.`] : []),
    "Для того чтобы помочь Вам, мне нужно уточнить несколько вопросов:",
    "1. Какая услуга нужна: стрижка, окрашивание, укладка или барбер-сервис?",
    locationLabel ? "2. Локацию фиксирую по контексту или скорректируете город/регион?" : "2. Какой город/регион приоритетный?",
...
function buildRetailBreadSinglePieceReply(message = ""): string {
  const source = oneLine(message || "");
  const normalized = normalizeComparableText(source);
  const geo = detectGeoHints(source);
  const locationLabel = formatGeoScopeLabel(geo.city || geo.region || "");
  ...
  if (commodityTag && commoditySlotState) {
    ...
    if (commodityTag === "milk") {
      ...
      if (missingMilkParams.length > 0) {
        questions.push(`Уточните параметры молока: ${missingMilkParams.join(", ")}.`);
      }
    } else {
    ...
    hasLeisureEveningIntent
      ...
      : (
          hasExplicitServiceIntent && !hasExplicitProductIntent
            ...
            : (
                hasExplicitProductIntent && !hasExplicitServiceIntent && !focus
                  ? "Что именно нужно найти по товару (можно 2-3 ключевых слова: тип, характеристики, фасовка)?"
                  : hasExplicitProductIntent && !hasExplicitServiceIntent && focus
                    ? hasFishCommodityIntent
                      ? "Какой вид рыбы нужен: свежая/охлажденная/замороженная, и какая порода/категория?"
                      : null // Товар уже указан в запросе, не спрашиваем повторно
  ...
  if (
    ...
    questions.push(
      hasExplicitProductIntent && !hasExplicitServiceIntent
        ? "Какие условия обязательны: объем/фасовка, подтвержденные контакты, наличие телефона/сайта?"
        : "Какие условия обязательны: формат работы, подтвержденные контакты, наличие телефона/сайта?",
    );
  }
...
function buildGreetingCapabilitiesReply(): string {
  return [
    ...
    `Я помогаю по интерактивному справочно-информационному порталу ${PORTAL_BRAND_NAME_RU}.`,
    "Чем могу помочь:",
    "1. Подобрать компании по критериям (что продаете/покупаете, город/регион, рубрика).",
    "2. Составить текст/запрос поставщику или подрядчику (электронная почта, мессенджеры).",
...
function buildCapabilitiesBoundaryReply(): string {
  return "В моей компетенции только то, о чем я сказал. Но со временем список моих услуг может расти";
}

function buildHardFormattedReply(message: string, history: AssistantHistoryMessage[] = []): string | null {
  if (looksLikeRetailBreadSinglePieceRequest(message)) return buildRetailBreadSinglePieceReply(message);
  ...
  if (looksLikeDiningPlaceIntent(message)) {
    ...
    const geo = detectGeoHints(diningSeed || message);
    return buildDiningCityClarifyingReply({
      locationHint: geo.city || geo.region || null,
      seedText: diningSeed || message,
      currentText: message,
    });
  }
...
function pickPrimaryRubricHintForClarification(params: {
  ...
  return ranked[0]?.hint || hints[0] || null;
}
...
function looksLikeCultureVenueDistractorReply(text: string): boolean {
  ...
  const hasCultureCompanyCue =
    /(театр\p{L}*|драмтеатр\p{L}*|опер\p{L}*|балет\p{L}*|филармон\p{L}*|концерт\p{L}*|кинотеатр\p{L}*|киносеанс\p{L}*|сеанс\p{L}*|афиш\p{L}*|музе\p{L}*|культур\p{L}*)/u.test(
      normalized,
    );
  if (hasCultureCompanyCue) return false;

  return /(поликлиник\p{L}*|больниц\p{L}*|медицин\p{L}*|ремонтн\p{L}*\s+завод|строительн\p{L}*|аренд\p{L}*\s+строительн\p{L}*|транспортн\p{L}*|машиностроен\p{L}*|автосервис\p{L}*|шиномонтаж\p{L}*|грузоперевоз\p{L}*|экспедир\p{L}*|логист\p{L}*|сварк\p{L}*|металлопрокат\p{L}*|металлоконструкц\p{L}*)/u.test(
    normalized,
  );
}
...
function buildRubricCountClarifyingQuestionsReply(params: {
  ...
  const normalizedSeed = normalizeComparableText(seed || params.message || "");
  const locationHint = formatGeoScopeLabel(params.vendorLookupContext?.city || params.vendorLookupContext?.region || "");
  const rubricName = truncate(oneLine(hint.name || ""), 90);
  const categoryName = truncate(
    oneLine(hint && typeof hint === "object" && "category_name" in hint ? String((hint as any).category_name || "") : ""),
    90,
  );
  const rubricLabel = hint.type === "rubric" ? rubricName : categoryName || rubricName;
  const rubricUrl = truncate(oneLine(hint.url || ""), 180);
  const filteredCardsLink = buildCatalogFilteredSearchLink({
    commodityTag,
    city: params.vendorLookupContext?.city || null,
    region: params.vendorLookupContext?.region || null,
  ...
  const hasExplicitServiceIntent = hasExplicitServiceIntentByTerms(normalizedSeed);
  const inferredFocus = normalizeFocusSummaryText(summarizeSourcingFocus(seed));
  const hasProductFocus = Boolean(commodityTag || inferredFocus);
  const rubricScopeLabel = hint.type === "rubric" ? "подрубрике" : "рубрике";

  const lines: string[] = [];
  if (count > 0) {
    if (commodityTag === "footwear") {
      lines.push(`На портале в ${rubricScopeLabel} «${rubricLabel}» сейчас ${count} ${formatCompaniesNounRu(count)}, которые занимаются обувью.`);
    } else {
      lines.push(`На портале в ${rubricScopeLabel} «${rubricLabel}» сейчас ${count} ${formatCompaniesNounRu(count)}.`);
    }
  } else {
    lines.push(`На портале в ${rubricScopeLabel} «${rubricLabel}» собраны профильные компании по вашему запросу.`);
  }
  if (rubricUrl) lines.push(`Рубрика портала: ${rubricUrl}`);
  if (filteredCardsLink) lines.push(`Открыть карточки с фильтром: ${filteredCardsLink}`);
  lines.push("Для того чтобы помочь Вам, мне нужно уточнить несколько вопросов:");

  if (commodityTag === "footwear") {
    lines.push("1. Какой тип обуви нужен: женская/мужская/детская, повседневная/спортивная/рабочая?");
    lines.push(`2. Какой город/регион приоритетный${locationHint ? ` (сейчас вижу: ${locationHint})` : ""}?`);
    lines.push("3. Вам нужна розница или опт?");
    lines.push("4. Есть ли требования по бренду/материалу?");
  } else if (hasExplicitProductIntent && !hasExplicitServiceIntent) {
    const productQuestions: string[] = [];
    if (hasMetalRolling && !hasConcreteMetalRollingItem) {
      productQuestions.push("Что именно нужно купить из металлопроката: лист, труба, арматура, уголок, швеллер или другое?");
    }
    if (!hasWholesaleRetailIntent && !hasExplicitQty && !hasSinglePieceRetailIntent) {
      productQuestions.push("Покупка нужна оптом или в розницу?");
    }
    // Не спрашиваем о параметрах товара, если товар уже указан в запросе
    if (!hasProductFocus) {
      productQuestions.push("Какие параметры товара обязательны: тип/характеристики/фасовка?");
    }
    productQuestions.push(`Какой город/регион приоритетный${locationHint ? ` (сейчас вижу: ${locationHint})` : ""}?`);
    productQuestions.push("Какие условия обязательны: формат поставки, подтвержденные контакты, наличие телефона/сайта?");
    lines.push(...productQuestions.map((question, index) => `${index + 1}. ${question}`));
  } else if (hasExplicitServiceIntent && !hasExplicitProductIntent) {
    lines.push("1. Какую услугу нужно подобрать: вид услуги и обязательный результат?");
    lines.push(`2. Какой город/регион приоритетный${locationHint ? ` (сейчас вижу: ${locationHint})` : ""}?`);
    lines.push("3. Какие условия обязательны: формат работы, документы, подтвержденные контакты?");
  } else {
    lines.push("1. Что именно нужно найти (2-3 ключевых слова)?");
    lines.push(`2. Какой город/регион приоритетный${locationHint ? ` (сейчас вижу: ${locationHint})` : ""}?`);
    lines.push("3. Какие условия обязательны: формат работы, подтвержденные контакты, наличие телефона/сайта?");
  }

  lines.push("После ответа на вопросы сразу подберу релевантные компании по вашим критериям.");
  return lines.join("\n");
}
...
function enforceConfirmedRubricAdvice(params: { text: string; rubricHintItems: BiznesinfoRubricHint[]; seedText?: string | null }): string {
  let out = String(params.text || "").trim();
  if (!out) return out;
  if (!replyContainsRubricAdvice(out)) return out;

  const sectionHeaderPattern =
    /^\s*(?:\*\*)?(Подходящие\s+рубрики\s+для\s+отбора|Рубрики\s+для\s+старта|Категории\s+компаний\s+для\s+поиска\s+в\s+каталоге|Только\s+существующие\s+рубрики\s+портала(?:\s*\(проверено\s+по\s+каталогу\))?)(?:\*\*)?\s*:?\s*$/iu;
  ...
  const confirmedAppendix = buildConfirmedRubricHintsAppendix(relevantRubricHints, 4);
...
function postProcessAssistantReply(params: {
  ...
  const hardFormattedReply = buildHardFormattedReply(params.message || "", params.history || []);
...
async function fetchVendorCandidates(params: {
  ...
  if (params.allowBroadGeoFallback) {
    ...
    if (broadGeoRecall.length > 0) {
      ...
      const broadGeoFallback = applyDiningStreetScope(dedupeVendorCandidates(broadGeoRecall).slice(0, limit));
...
function buildAssistantSystemPrompt(): string {
  return [
    ...
    "- Для «где поесть / рестораны / кафе» это валидный интент: уточни локацию и формат, затем предложи релевантные карточки.",
...
function buildUploadedFilesContextBlock(params: {
  ...
  if (parsed.length > 0) {
    lines.push("");
    lines.push("Извлеченный текст из файлов (документы и изображения):");
...
export async function POST(request: Request) {
  ...
  const runProvider = async (opts: { signal?: AbortSignal; onDelta?: (_delta: string) => void; streamed: boolean }) => {
    ...
    const hardFormattedReply = buildHardFormattedReply(message, history);
